...
def fill_green_color(file, sheet_name, row_number, column_number):
    workbook = openpyxl.load_workbook(file)
    sheet = workbook[sheet_name]
    green_fill = PatternFill(start_color='60b212',
                             end_color='60b212',
                             fill_type='solid')
    sheet.cell(row_number, column_number).fill = green_fill
    workbook.save(file)


def fill_red_color(file, sheet_name, row_number, column_number):
    workbook = openpyxl.load_workbook(file)
    sheet = workbook[sheet_name]
    red_fill = PatternFill(start_color='ff0000',
                           end_color='ff0000',
                           fill_type='solid')
    sheet.cell(row_number, column_number).fill = red_fill
    workbook.save(file)
